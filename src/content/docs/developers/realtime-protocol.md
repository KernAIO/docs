...
Clients keep **one WebSocket** to the chat service (`/ws`) for all workspaces. Messages are JSON, validated with Zod schemas from `@kernhq/contracts` (`ClientMessage`, `ServerMessage`).
